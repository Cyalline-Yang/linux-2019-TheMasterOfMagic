...
anon_do "mkdir impossible_dir" &> /dev/null
[[ $? -ne 0 ]] || exit_because "failed to prevent anonymous user from writing"
...
norm_do "mkdir new_dir" &> /dev/null
[[ $? -eq 0 ]] || exit_because "failed to create a new directory as normal user"
